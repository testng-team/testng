...
public class RetryAnalyzerWithComplexDataProviderTest extends SimpleBaseTest {
    ...
    // Test with string array of objects in data-provider with end result as success after 3 retry attempts
    @Test(description = "GITHUB-2148")
    ...
    // Test with multiple integer arrays in data-provider when parallel mode on and end result as success
    ...
    public void testWithSingleParam() {
        ...
        assertEquals(tla.getFailedTests().size(), 0);
        assertEquals(tla.getSkippedTests().size(), 6); // Number of total skipped count should be 6, each for a test
    }
