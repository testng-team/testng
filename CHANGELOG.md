...
- [GITHUB-1164](https://github.com/testng-team/testng/issues/1164): A third-party library can now own a test method parameter, through the new org.testng.IParameterResolver. It is an ITestNGListener, so it is registered the way every other listener is -- @Listeners, the `<listeners>` tag, TestNG.addListener, or the ServiceLoader -- and it answers two questions: supportsParameter(Parameter, ITestNGMethod, ITestContext), asked while the arguments of an invocation are built, and resolveParameter(...), asked once per invocation. A parameter a resolver claims is taken out of data provider matching rather than padded: @Test(dataProvider = "dp") void test(String a, @Mine Custom c, int b) runs against a row of {"value", 42} and is invoked with ["value", resolved, 42]. The same resolution applies to a method with no data provider at all, which is the half of #1164 that used to fail with "Cannot inject @Test annotated Method", and it composes with the varargs matching and with native injection: the first Method parameter, every ITestContext, ITestResult and XmlTest one, and an `Object[]` slot, is never offered to a resolver, so one cannot displace what TestNG already owns, while a second Method parameter -- which TestNG has never injected -- is offered like any other. A retry is an invocation of its own and is resolved afresh, whether or not its data provider row is cached; a data provider row that runs in parallel is resolved on the thread that invokes it; and a method that is only reported as skipped resolves nothing. A parameter carrying @NoInjection is offered, since that annotation already means TestNG has stopped owning it, and falls through to the data provider as before when no resolver wants it. Two resolvers claiming the same parameter is an error naming the method, the parameter and both resolvers, rather than an undocumented first-listener-wins; a resolver that throws, or that answers a value the parameter cannot take, fails that method with a diagnostic naming the resolver, the method and the parameter, and keeps the original exception as the cause. A parameter no resolver claims keeps the matching rules and the diagnostics it had. Resolvers are held per suite, beside the data provider listeners, and a run that registers none resolves nothing -- the resolution is the empty singleton and every filter and injection short circuits on it, so parameter matching is bit for bit what it was. This first implementation covers @Test methods; constructors, @Factory and the configuration methods are not resolved. They are listed under Possible backward incompatible changes below (Julien Herr)
...
- alwaysRun on a @BeforeSuite, @BeforeTest, @BeforeClass, @BeforeMethod or @BeforeGroups method no longer runs it after a previous configuration failed: the method is reported as skipped. An alwaysRun @After method still runs, so an alwaysRun @AfterMethod paired with an alwaysRun @BeforeMethod now runs after a setup that did not, and must not assume what that setup left behind: a teardown that closes what its setup opened has to check that it was opened. See [GITHUB-1622](https://github.com/testng-team/testng/issues/1622) above.
...
- Under parallel="tests", ITestNGMethod.getTimeOut() answers 0 for a method that declares no time-out of its own, in a `<test>` that declares none either, when the suite does: it used to answer the suite value. A listener or a reporter reading that value sees the change; the run itself does not. What a method that overruns the suite time-out reports was measured identical before and after -- the `<test>` worker is cancelled at the suite bound in both, which is what SuiteRunner has always done with that value, and the per-method time-out it used to carry as well fired at the same instant.
- org.testng.internal.invokers.ITestInvoker declares getParameterResolvers() and failRowThatDoesNotFit(ITestNGMethod, `Object[]`, TestNGException). An implementation outside TestNG has to add both; TestInvoker is the only one in TestNG, and a caller of the interface is unaffected.
- The constructors of org.testng.internal.invokers.Invoker, and of the package private TestInvoker and ParameterHandler, take the suite's IParameterResolver holder. Both packages are internal and OSGi exported, so a direct caller has to pass one; org.testng.internal.ParameterResolverHolder is what SuiteRunner builds.
- org.testng.internal.reflect.ReflectionRecipes.isInstanceOf declares its second parameter @Nullable, and MethodMatcherContext gained a five-argument constructor beside the existing four-argument one. isInstanceOf has always answered for a null value -- a null is an instance of any reference type and of no primitive one -- so the annotation states what the body already did. It is source and binary compatible for a Java caller; a Kotlin caller gains the ability to pass null.
...
- org.testng.internal.Parameters.injectParameters(`Object[]`, Method, ITestContext), the form TestNG's own invoker used until now. It knows nothing of the IParameterResolvers a run registers, so a parameter one of them owns is counted against the data provider row and the method fails on arity with nothing saying resolution was skipped. injectParameters(`Object[]`, ITestNGMethod, ITestContext, Collection) is what TestNG calls (Julien Herr)

### Fixed

- [GITHUB-3537](https://github.com/testng-team/testng/issues/3537): Dry run reports tests with expectedExceptions as passed without invoking them. The missing-exception check still applies to normal execution (Lev Shagalov)
- [GITHUB-3435](https://github.com/testng-team/testng/issues/3435): A @Factory that declares @BeforeMethod or @AfterMethod no longer scans every instance's configuration on each test invocation. TestClass indexes those methods by instance id, the same way it already indexes @BeforeClass and @AfterClass. The invoker reads that index. The public flat accessors still return every instance (Burak Kalaycı)
- [GITHUB-3530](https://github.com/testng-team/testng/issues/3530): A later configuration failure for the same class instance no longer erases earlier records. ConfigInvoker kept one RecordedFailure per instance, so a parallel worker's teardown could replace another worker's setup failure. A retry on the later thread then ignored that replacement as its own and ran even though the earlier failure still applied. Each key now keeps the latest record per thread, which is what the retry filter needs, rather than appending every skipped configuration onto a CopyOnWriteArrayList. org.testng.conffailure.RetriedInvocationTest covers two parallel data-provider rows that force that order (Burak Kalaycı)
- [GITHUB-1622](https://github.com/testng-team/testng/issues/1622): alwaysRun on a @Before method no longer runs it after a previous configuration failed. The attribute is documented to mean two different things -- on a @Before method it lets the method run regardless of the groups it belongs to, on an @After method it lets the method run even if a method invoked before it failed or was skipped -- and since 6.9.5 a @Before method had been getting both. The fix for [GITHUB-420](https://github.com/testng-team/testng/issues/420) had added the four getBefore\*() terms to MethodHelper.isAlwaysRun, and ConfigInvoker read that one predicate for two independent decisions: whether a configuration of a @Test(enabled = false) class may still run, which is what [GITHUB-420](https://github.com/testng-team/testng/issues/420) needed, and whether it may run past a failure, which it did not. So a failing @BeforeSuite -- which marks the whole suite failed -- was followed by every alwaysRun @BeforeTest, @BeforeClass, @BeforeMethod and @BeforeGroups of the run, and by every other alwaysRun @BeforeSuite: the reporter's four-method class reported 4 configuration failures and 0 skips where 6.9.4 had reported 1 and 3. The failure bypass is now read from a separate predicate, MethodHelper.canBypassConfigurationFailure, which answers true for the after role only -- @AfterSuite, @AfterTest, @AfterClass, @AfterMethod and @AfterGroups -- so those are unchanged and keep running after a failure. The role is the one TestNG runs the method in, not the merged annotation, so a method that carries both a @BeforeMethod and an @AfterMethod(alwaysRun = true) is skipped as a setup and run as a teardown. A skipped setup still marks what it would have set up, so the tests that need it are skipped too, as they are when the setup does not say alwaysRun. The group meaning of alwaysRun on a @Before method is untouched, and so is the enabled decision [GITHUB-420](https://github.com/testng-team/testng/issues/420) covers. This is deliberately not tied to configFailurePolicy: an alwaysRun @BeforeMethod is skipped after a failed @BeforeClass under SKIP and under CONTINUE alike. It is listed under Possible backward incompatible changes below (Julien Herr)
- A configuration method skipped because an earlier one failed no longer moves the level flags that decide where ignoreFailure is looked up. The skip went through the same bookkeeping as a failure, flags included, so a skipped @BeforeMethod in one class made a later class's @BeforeClass(ignoreFailure = true) stop protecting that class: the lookup read the later class's @BeforeMethod list instead of its @BeforeClass list, found no ignoreFailure there, and skipped the test. A skip still marks what the setup would have set up, so the tests that need it are skipped; only the flags are left to a failure. [GITHUB-1622](https://github.com/testng-team/testng/issues/1622) above makes such skips far more frequent, since an alwaysRun @Before method used to run instead (Julien Herr)
- An attempt that retries a failed test method is no longer held back by what the failed attempt recorded, and by nothing else. TestInvoker already exempted the retried test method from every recorded configuration failure, but not its setup, so a retry whose @AfterMethod had failed ran the test method with no setup at all; and the exemption was a flag on the retry context, which MethodRunner hands to the data provider rows that follow, so those rows ran past their own configuration failures too. ConfigInvoker now numbers every failure it records, with the thread that recorded it; a retry takes the mark before it starts, and its setup, its test method and its teardown ignore the failures its own thread recorded up to that mark only -- which is what the attempt it retries recorded, since the retry runs on that attempt's thread right after it. A setup that fails during the retry still skips the setups and the test method after it; a failure another data provider row recorded on another thread before the retry started still counts, so the retry's setup and test method are skipped where the test method used to run without its setup; and the rows that follow the retry are back to reporting what they report when nothing is retried. Until the [GITHUB-1622](https://github.com/testng-team/testng/issues/1622) fix in this release the missing setup was only reachable for a @BeforeMethod without alwaysRun, since alwaysRun used to bypass the failure and hide it (Julien Herr)
- [GITHUB-3522](https://github.com/testng-team/testng/issues/3522): ignoreFailure on a @BeforeMethod no longer lets a later class run a test after that class's own @BeforeClass failed. The flags that pick which configuration list to scan for ignoreFailure are now per test class and instance, keyed by the class under test, not the declaring type of the configuration method. A real @BeforeMethod failure in class Z used to make class A's @BeforeMethod(ignoreFailure = true) look like the failure that applied to A, so A.aTest ran with its setup skipped. The same leak happened across sibling subclasses that share a configuration method, across implementors of an interface default, and across @Factory instances of one class. org.testng.conffailure.issue3522.Issue3522Test covers A alone, Z then A in one `<test>`, the sibling and interface cases, and two factory instances (Burak Kalaycı)
- [GITHUB-3513](https://github.com/testng-team/testng/issues/3513): Under parallel="tests", a suite time-out that cancels a `<test>` worker now reports the methods that did not return, and the run exits non-zero. invokeAll interrupts the worker and returns; a method that ignores interruption keeps running, runTest never stores a SuiteResult, and ExitCodeListener then sees the `<test>` tags that finished -- so a suite that still has a method in flight exits 0 and writes no testng-failed.xml. After the pool returns, SuiteRunner waits a short interval for workers that respect interruption to record themselves, then fails every unfinished invocation with ThreadTimeoutException and stores the `<test>`. The same suite with parallel="none" already reported the method as failed. org.testng.timeout.issue3513.Issue3513Test covers a method that ignores interruption and one that stops when interrupted, each beside an empty `<test>` that still passes (Burak Kalaycı)
...
- [GITHUB-1333](https://github.com/testng-team/testng/issues/1333): A suite-level time-out no longer costs parallel="tests" its thread affinity. Under parallel="tests" a time-out declared on `<suite>` is no longer inherited as a time-out on each method of a `<test>`; it bounds the `<test>`, which is what SuiteRunner already did with it. The DTD has always given the attribute one meaning per mode -- it aborts "the method (if parallel=methods) or the test (if parallel=tests)" -- but BaseTestMethod.getTimeOut fell back to the XmlTest value in every mode, so under parallel="tests" the suite value was applied twice: once to the `<test>` worker, by SuiteRunner, and again to every method. That second application is what caused the reported symptom: a method carrying a time-out is handed to a single-use executor of its own, so a `<suite time-out=...>` put the @BeforeClass and each @Test of a `<test>` on three different threads, and removing the time-out was the only way to get the documented behaviour back. The inheritance is skipped exactly when the suite-level bound applies and carries the same value: the suite runs parallel="tests", and the `<test>` declares no time-out of its own, or the same one. Anything else still reaches the method as before, because nothing else enforces it: a time-out declared on the `<test>` alone, since SuiteRunner bounds its workers with the suite value only and TestTaskExecutor is not used in a mode that is not parallel at test level; and a parallel="tests" declared on the `<test>` alone, since the parallel-tests path is selected from the suite's mode. Both keep their per-method time-out, and so keep running a timed method on a thread of its own. An explicit @Test(timeOut=...) is untouched and still bounds the method in every mode. Every other parallel mode still inherits the suite and test time-out per method exactly as before. Two shapes keep a pool of their own whatever the time-out says, and are unchanged here: a parallel data provider, and @Test(invocationCount=N, threadPoolSize=N). They are listed under Possible backward incompatible changes below (Julien Herr)
- A retry that re-reads its data provider row now invokes the test method with the same kind of arguments the first attempt had. TestInvoker.retryFailed re-reads the row when the data provider declares cacheDataForTestRetries = false, and handed it to the invocation as it came, without running it back through the matcher -- so everything TestNG supplies itself was dropped and the method was called with the provider's arity instead of its own. A method taking a natively injected ITestContext beside one provider value failed its retry with "wrong number of arguments: 1 expected: 2", having never entered the body. The row now goes back through Parameters.injectParameters, which is what produced the first attempt's arguments, so native injections and the parameters an IParameterResolver owns are both restored at their declared positions. A retry whose row an interceptor excluded still falls back to the values it came in with, unchanged (Julien Herr)
- A retry whose data provider fails to rebuild its row now reports that failure. With cacheDataForTestRetries = false, TestInvoker.retryFailed asks the provider again, and when that threw it left the retry loop with nothing recorded: the method's first attempt had already been marked as retried, so a method that failed and whose provider then broke was reported as one skip and no failure. The provider's exception is now the retry's own result, given the status a first attempt would have been given -- a skip, or a failure when the configuration or the provider asks for data provider failures to fail the test (Julien Herr)
- A parallel data provider whose row does not fit the method now fails that row alone, and runs the others. The rows of a parallel provider used to be matched against the method one after the other on the scheduling thread, before any was submitted, so the first row that did not fit ended the whole method with a MethodMatcherException and no row ran. Each row is now matched on the worker that will invoke it, and a row that does not fit is reported as its own failure, carrying the row and the matcher's exception as it was thrown -- not a CompletionException around it, which is what letting the worker throw would have handed a listener. The rows that fit run and pass. A sequential provider is unchanged: it still stops at the first row that does not fit (Julien Herr)
...
Cut from the `release_7.5` branch, which forked before the 7.5 tag. This release is not 7.5 plus the fix below: it is missing 20 commits that 7.5 has, among them [GITHUB-2701](https://github.com/testng-team/testng/issues/2701) and [GITHUB-2646](https://github.com/testng-team/testng/issues/2646), both listed under 7.5. Moving from 7.5 to 7.5.1 for the security fix gives those up.
...
- GITHUB-2825: Programmatically Loading TestNG Suite from JAR File Fails to Delete Temporary Copy of Suite File (Steven Jubb)
...
- GITHUB-2802: some methods' javadoc in ISuiteListener and ITestListener is misleading (Yehui Wang)
...
- GITHUB-1173: Parallel="classes" executes methods of test class in different threads (Krishnan Mahadevan)
...
- GITHUB-137: Main parameters with a default value should be overridden if a main parameter is specified
- GITHUB-107: Allow enum values without converting them to uppercase.
...
- **Eclipse 5.11.0.19:** New "parallel" preference setting (Windows / Preferences / TestNG)
...
- Allow IObjectFactory to load from non-standard classloader (for PowerMock support)
- **Eclipse 5.11.0.19:** IIinvokedMethodListeners were not invoked
...
- **Eclipse plug-in:** TESTNG-98 (temporary files have guaranteed fixed names)
- **Eclipse plug-in:** TESTNG-95 (Assertion failed comparison trims trailing ">")
- **Eclipse plug-in:** TESTNG-70 (TestNG prevents eclipse from opening an older CVS version of a java class)
- **Eclipse plug-in:** display of test hierarchy information (TESTNG-29)
