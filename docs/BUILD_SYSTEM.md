...
|---------|--------------|---------------|------|
| **Running Gradle** | Java 25 | Default JDK on system | Latest LTS, runs Gradle 9, required by nmcp plugin (needs 17+) |
| **Compiling Code** | Java 25 | `jdkBuildVersion=25` | Latest LTS toolchain for compilation |
| **Target Bytecode** | Java 11 | `targetJavaVersion=11` | TestNG artifacts are Java 11 compatible |
| **Running Tests** | Java 11/17/21/25/26, plus 27/28 (EA) | `jdkTestVersion=X` | Verify compatibility across Java versions |

### Why Java 25 is Used for ALL Builds

* Gradle 9 runs on JDK 25.
* The `com.gradleup.nmcp.aggregation` plugin (used for Maven Central publishing) requires Java 17+.
* We use Java 25 (latest LTS) for modern tooling and features.
...
**Result:** You must use Java 25 to run any Gradle command, including `./gradlew build`, `./gradlew test`, etc.
...
```bash
# Java Versions
-PjdkBuildVersion=25          # JDK version for building (default: 25)
...
│  ┌──────────────────────────────────────────┐          │
│  │ Task: compileJava                        │          │
│  │ Toolchain: Java 25 (jdkBuildVersion)     │          │
...
- **Gradle runs with:** The default Java on your system (must be 25)
- **Code compiles with:** Java 25 toolchain (via `jdkBuildVersion`)
...
The matrix is generated by `.github/workflows/matrix.mjs`:

- **Java versions**: 11, 17, 21, 25, 26, plus 27 and 28 (EA)
...
1. **Two Java versions are installed**: The matrix version (e.g., 11) AND Java 25
2. **Java 25 is listed last**, so it becomes the default
3. **Gradle runs with Java 25** (latest LTS)
...
- name: Test
  ...
  with:
    arguments: build
    properties: |
      jdkBuildVersion=25              # Compile with Java 25
