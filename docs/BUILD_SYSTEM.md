...
|---------|--------------|---------------|------|
| **Running Gradle** | Java 21 | Default JDK on system | Latest LTS, required by nmcp plugin (needs 17+) |
| **Compiling Code** | Java 21 | `jdkBuildVersion=21` | Latest LTS toolchain for compilation |
| **Target Bytecode** | Java 11 | `targetJavaVersion=11` | TestNG artifacts are Java 11 compatible |
| **Running Tests** | Java 11/17/21/25/26 | `jdkTestVersion=X` | Verify compatibility across Java versions |

### Why Java 21 is Used for ALL Builds

* The `com.gradleup.nmcp.aggregation` plugin (used for Maven Central publishing) requires Java 17+.
* We use Java 21 (latest LTS) for modern tooling and features.
...
**Result:** You must use Java 21 to run any Gradle command, including `./gradlew build`, `./gradlew test`, etc.
...
```bash
# Java Versions
-PjdkBuildVersion=21          # JDK version for building (default: 21)
...
│  ┌──────────────────────────────────────────┐          │
│  │ Task: compileJava                        │          │
│  │ Toolchain: Java 21 (jdkBuildVersion)     │          │
...
- **Gradle runs with:** The default Java on your system (must be 21)
- **Code compiles with:** Java 21 toolchain (via `jdkBuildVersion`)
...
- **Java versions**: 11, 17, 21, 25, 26 (EA)
...
1. **Two Java versions are installed**: The matrix version (e.g., 11) AND Java 21
2. **Java 21 is listed last**, so it becomes the default
3. **Gradle runs with Java 21** (latest LTS)
...
- name: Test
  ...
  with:
    arguments: build
    properties: |
      jdkBuildVersion=21              # Compile with Java 21
