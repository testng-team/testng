...
public class AnnotationTestConverter {
	...
	private Class expectedValueTypeForKey(DocletTag tag, String key) {
		...
		if (annotationClass==null) {
			ppp("Found unknown testng annotation "+tag.getName()+" in file "+tag.getContext().getSource().getFile().getAbsolutePath()+" at line "+tag.getLineNumber());
		...
		try {
			...
			ppp("Found unknown testng parameter "+key+" in annotation "+tag.getName()+" in file "+tag.getContext().getSource().getFile().getAbsolutePath()+" at line "+tag.getLineNumber());
	...
	private String suffixForClass(Class type) {
		...
		if (Class.class.isAssignableFrom(type)) return ".class";
		return "";
	}
	...
	private Object prefixForClass(Class type) {
		if (String.class.isAssignableFrom(type)) return "\"";
		if (Class.class.isAssignableFrom(type)) return "";
		return "";
	}
	...
	private List<String> insertAnnotations(JavaSource source,List<String> lines) {
		...
		if (oldLinesLength==lines.size()) return lines;
	...
	private void writeFile(File filePath,String packageName, List<String> lines) {
		...
		File file = m_outDir==null?filePath:new File(getPackageOutputDir(m_outDir, packageName), fileName);
