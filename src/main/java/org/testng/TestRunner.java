...
//  private Map<Class<?>, ITestClass> m_classMap = Maps.newLinkedHashMap();
  private Map<XmlClass, ITestClass> m_classMap = Maps.newLinkedHashMap();
  /**
...
//    m_testClassFinder= new TestNGClassFinder(classMap,Maps.<Class<?>, List<Object>>newHashMap(),
    m_testClassFinder = new TestNGClassFinder(classMap, Maps.<XmlClass, List<Object>>newHashMap(),
...
//      m_classMap.put(ic.getRealClass(), tc);
      m_classMap.put(ic.getXmlClass(), tc);
    }
...
//    final Set<Class<?>> classes = cim.getClasses();
    final Set<XmlClass> classes = cim.getXmlClasses();
...
//        for(Class<?> tc: classes) {
        for (XmlClass tc : classes) {
//          List<XmlInclude> includedMethods = cim.getXmlClass(tc).getIncludedMethods();
          List<XmlInclude> includedMethods = tc.getIncludedMethods();
  ...
  public List<IWorker<ITestNGMethod>> createWorkers(List<ITestNGMethod> methods) {
    methods.sort(Comparator.comparing(tm -> tm.getXmlClass().getIndex() * 100000 + tm.getIndex()));
