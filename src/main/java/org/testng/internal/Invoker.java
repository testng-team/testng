...
public class Invoker implements IInvoker {
	 ...
	 * If instance is non-null, the configuration will be run on it. If it is
	 * null,
	...
	/**
	 * Marks the current <code>TestResult</code> as skipped and invokes the
	 * listeners.
	 */
	...
	/**
	 * Record internally the failure of a Configuration, so that we can
	 * determine
	 ...
	 * TODO: Should change this method to be more like invokeMethod() so that we
	 * can
	...
	private void invokeConfigurationMethod(Object targetInstance,
...

		{
			...
			try {
				...
				//
				IConfigurable configurableInstance = IConfigurable.class.isAssignableFrom(tm.getMethod().getDeclaringClass()) ? (IConfigurable)targetInstance : m_configuration.getConfigurable();
	...
	private ITestResult invokeMethod(Object instance,
		...
		try {
			...
			ExpectedExceptionsHolder expectedExceptionClasses = new ExpectedExceptionsHolder(m_annotationFinder, tm, new RegexpExpectedExceptionsHolder(m_annotationFinder, tm));
	...
	/**
	 * The array of methods contains @BeforeMethods if isBefore if
	 * true, @AfterMethods
	 * otherwise. This function removes all the methods that should not be run
	 * at this
	 * point because they are either firstTimeOnly or lastTimeOnly and we
	 * haven't reached
	...
	/**
	 * invokeTestMethods() eventually converge here to invoke a single @Test
	 * method.
	 * <p/>
	 * This method is responsible for actually invoking the method. It decides
	 * if the invocation
	 * must be done:
	 * <ul>
	 ...
	 * invoking @BeforeGroup, @BeforeMethod, @AfterMethod, @AfterGroup
	...
	int retryFailed(Object instance,
		...
		do {
			...
			/**
			 * TODO: This recreates all the parameters every time when we only
			 * need
			 * one specific set. Should optimize it by only recreating the set
			 * needed.
			 */
	 ...
	 * Note (alex): this method can be refactored to use a
	 * SingleTestMethodWorker that
	 * directly invokes
	 ...
	 * and this would simplify the implementation (see how DataTestMethodWorker
	 * is used)
	 */
	@Override
	public List<ITestResult> invokeTestMethods(ITestNGMethod testMethod,
		...
		final Map<String, String> parameters = testMethod.findMethodParameters(testContext.getCurrentXmlTest());
		...
		while (invocationCount-- > 0) {
			...
			//
				...
				try {
					...
					if (bag.parameterHolder.origin == ParameterOrigin.ORIGIN_DATA_PROVIDER &&
						...
						while (allParameterValues.hasNext()) {
							Object[] parameterValues = injectParameters(allParameterValues.next(),
									testMethod.getMethod(), testContext, null /*
																				 * test
																				 * result
																				 */);
							TestMethodWithDataProviderMethodWorker w = new TestMethodWithDataProviderMethodWorker(this,
					...
					ITestResult r = new TestResult(testMethod.getTestClass(),
							instance,
							testMethod,
							cause,
							start,
	...
	private ITestResult registerSkippedTestResult(ITestNGMethod testMethod, Object instance,
		...
		ITestResult result = new TestResult(testMethod.getTestClass(),
				instance,
				testMethod,
				throwable,
				start,
	...
	/**
	 * Gets an array of parameter values returned by data provider or the ones
	 * that
	 * are injected based on parameter type. The method also checks for
	 * {@code NoInjection}
	 * annotation
	 *
	...
	/**
	 * To reduce thread contention and also to correctly handle
	 * thread-confinement
	 * this method invokes the @BeforeGroups and @AfterGroups corresponding to
	...
	private String checkDependencies(ITestNGMethod testMethod,
		...
		if (null != groups && groups.length > 0) {
			...
			for (String element: groups) {
				ITestNGMethod[] methods = MethodGroupsHelper.findMethodsThatBelongToGroup(testMethod,
	...
	/**
	 * @return the test results that apply to one of the instances of the
	 *         testMethod.
	 */
	...
	/**
	 * This class holds a {@code ParameterHolder} or in case of an error, a
	 * non-null
