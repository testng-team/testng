 ...
 *
 * Test Case Summary: Parallel by methods mode with sequential test suites using a factory but no
 * dependencies and no data providers.
 *
 * Scenario Description: Two suites with 1 and 2 tests respectively. One suite with two tests has a
 * test consisting of a single test class without a factory while the other consists of factories
 ...
 *
 * 1) For both suites, the thread count and parallel mode are specified at the suite level 2) The
...
public class ParallelByMethodsTestCase5Scenario2 extends BaseParallelizationTest {
  ...
  public void sanityCheck() {
    assertEquals(suiteLevelEventLogs.size(), 4,
        "There should be 4 suite level events logged for " + SUITE_A +
            " and " + SUITE_B + ": " + suiteLevelEventLogs);
    assertEquals(testLevelEventLogs.size(), 6,
        "There should be 6 test level events logged for " + SUITE_A +
            " and " + SUITE_B + ": " + testLevelEventLogs);

    assertEquals(testMethodLevelEventLogs.size(), 303,
