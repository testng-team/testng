 ...
 *
 * Test Case Summary: Parallel by methods mode with sequential test suites, no dependencies and no
 ...
 *
 * 1) For one of the suites, the thread count and parallel mode are specified at the suite level 2)
 * For one of the suites, the thread count and parallel mode are specified at the test level 3) For
 * one of the suites, the parallel mode is specified at the suite level, and the thread counts are
 * specified at the test level (thread counts for each test differ) 4) The thread count is less than
 * the number of test methods for the tests in two of the suites, so some methods will have to wait
 * the active thread count to drop below the maximum thread count before they can begin execution.
 * 5) The thread count is more than the number of test methods for the tests in one of the suites,
 * ensuring that none of the methods in that suite should have to wait for any other method to
 * complete execution 6) There are NO configuration methods 7) All test methods pass 8) NO ordering
 * is specified 9) group-by-instances is NOT set 10) There are no method exclusions
 */
