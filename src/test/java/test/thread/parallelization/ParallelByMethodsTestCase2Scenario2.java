 ...
 *
 * Test Case Summary: Parallel by methods mode with parallel test suites, no dependencies and no
 * factories or data providers.
 *
 * Scenario Description: Three suites with 1, 2 and 3 tests respectively. One test for a suite shall
 ...
 * the suites, so some methods will have to wait the active thread count to drop below the maximum
 * thread count before they can begin execution. 6) The thread count is more than the number of test
 * methods for the tests in one of the suites, ensuring that none of the methods in that suite
 * should have to wait for any other method to complete execution 7) There are NO configuration
 * methods 8) All test methods pass 9) NO ordering is specified 10) group-by-instances is NOT set
...
public class ParallelByMethodsTestCase2Scenario2 extends BaseParallelizationTest {
  ...
  public void sanityCheck() {
    ...
    assertEquals(testMethodLevelEventLogs.size(), 204,
    ...
    assertEquals(suiteOneTestMethodLevelEventLogs.size(), 33,
    ...
    assertEquals(suiteTwoTestMethodLevelEventLogs.size(), 54,
