 ...
 *
 * Scenario Description: Single suite with a single test consisting of a factory that provides two
 * instances of a single test class with five methods
 *
 * 1) Thread count and parallel mode are specified at the suite level 2) The thread count is equal
 * to the number of test methods times 2, the number of times each method will be invoked because
 * there are two instances of the test class. Expectation is that 10 threads will be spawned because
 * each method will run once for each of the two instances and that each instance of a test method
 * will run in separate threads. 3) There are NO configuration methods 4) All test methods pass 5)
...
public class ParallelByMethodsTestCase5Scenario1 extends BaseParallelizationTest {
  ...
  public void sanityCheck() {
    assertEquals(suiteLevelEventLogs.size(), 2,
        "There should be 2 suite level events logged for " + SUITE + ": " +
            suiteLevelEventLogs);
    assertEquals(testLevelEventLogs.size(), 2,
        "There should be 2 test level events logged for " + SUITE + ": " +
            testLevelEventLogs);
    assertEquals(testMethodLevelEventLogs.size(), 30,
  ...
  public void verifySuiteAndTestLevelEventsRunInSequentialOrderInSameThread() {
    ...
    verifyEventsOccurBetween(suiteListenerOnStartEventLog, testLevelEventLogs,
        ...
        "All of the test level event logs should have timestamps between the suite listener's onStart and "
            ...
            +
            ". Suite listener onFinish event log: " + suiteListenerOnFinishEventLog
            + ". Test level " +
  ...
  public void verifyTwoInstancesOfTestClassForAllTestMethods() {
    verifyNumberOfInstancesOfTestClassForMethods(SUITE, TEST,
        TestClassAFiveMethodsWithNoDepsSample.class, 2);
    verifySameInstancesOfTestClassAssociatedWithMethods(SUITE, TEST,
  ...
  public void verifyTestMethodLevelEventsAllOccurBetweenTestListenerStartAndFinish() {
    verifyEventsOccurBetween(testListenerOnStartEventLog, testMethodLevelEventLogs,
        ...
        "All of the test method level event logs should have timestamps between the test listener's onStart "
            ...
            ". Test Listener onFinish event log: " + testListenerOnFinishEventLog
            + ". Test method level " +
  ...
  public void verifyThatMethodLevelEventsRunInDifferentThreadsFromSuiteAndTestLevelEvents() {
    verifyEventThreadsSpawnedAfter(testListenerOnStartThreadId, testMethodLevelEventLogs,
        "All the thread IDs " +
  ...
  public void verifyThatAllEventsForATestMethodInClassInstanceExecuteInSameThread() {
    verifyEventsForTestMethodsRunInTheSameThread(TestClassAFiveMethodsWithNoDepsSample.class, SUITE,
        TEST);
  }
