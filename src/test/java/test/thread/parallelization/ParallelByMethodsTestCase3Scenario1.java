 ...
 *
 * Scenario Description: Single suite with a single test consisting of a single test class with five
 * methods with a data provider specifying 3 sets of data
 *
 * 1) Thread count and parallel mode are specified at the suite level 2) The thread count is equal
 * to the number of test methods times 3, the number of times each method will be invoked with a
 * data set from the data provider. Expectation is that only 5 threads will be spawned, one for each
 * of the methods and that in this thread, the method will be invoked 3 times, once for each set of
 * data from the data provider. 3) There are NO configuration methods 4) All test methods pass 5) NO
  ...
  public void sanityCheck() {
    ...
    assertEquals(testLevelEventLogs.size(), 2,
        "There should be 2 test level events logged for " + SUITE + ": " +
            testLevelEventLogs);
    assertEquals(testMethodLevelEventLogs.size(), 45,
