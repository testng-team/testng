...
public class TestListeners extends SimpleBaseTest {
  ...
  public void verifyWithoutListener() {
    ...
    testNG.setTestSuites(Collections
        .singletonList(getPathToResource("test/listeners/github1284/github1284_nolistener.xml")));
    testNG.run();
  ...
  public void verifyWithListener() {
    TestNG testNG = create();
    testNG.setTestSuites(Collections
        .singletonList(getPathToResource("test/listeners/github1284/github1284_withlistener.xml")));
    testNG.run();
    Assert.assertEquals(testNG.getStatus(), 0);
  }
