...
public class MAnnotationSampleTest {
  ...
  public void verifyTestClassLevel() {
    ...
    Assert.assertEqualsNoOrder(test1.getDependsOnGroups(), new String[]{"dg1", "dg2"},
        "depends on groups");
  ...
  public void verifyTestMethodLevel() throws SecurityException, NoSuchMethodException {
    ...
    Assert.assertTrue(test1.getEnabled());
    Assert.assertEqualsNoOrder(test1.getGroups(),
        new String[]{"group1", "group3", "group4", "group2"});
    Assert.assertTrue(test1.getAlwaysRun());
    Assert
        .assertEqualsNoOrder(test1.getDependsOnGroups(), new String[]{"dg1", "dg2", "dg3", "dg4"});
    Assert
        .assertEqualsNoOrder(test1.getDependsOnMethods(), new String[]{"dm1", "dm2", "dm3", "dm4"});
  ...
  public void verifyDataProvider() throws SecurityException, NoSuchMethodException {
    ...
    IDataProviderAnnotation dataProvider = m_finder
        .findAnnotation(method, IDataProviderAnnotation.class);
