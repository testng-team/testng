...
public class DistributedTest extends BaseDistributedTest {
  ...
  protected Thread startSlave(final String port) {
//    ppp("STARTING SLAVE ON " + port);
    Thread result = new Thread(new Runnable() {
      public void run() {
        TestNG.main(new String[] { "-slave", port, "-d", OutputDirectoryPatch.getOutputDirectory() });       
      }
    });
    result.setName("Slave-" + port);
    result.start();
    m_hostThreads.add(result);
    return result;
  }
  
  private File createHostProperties(String strategy)
  throws IOException 
  {
    ...
    p.setProperty("testng.strategy", strategy);
  ...
  private File m_hostFile;

//  @Configuration(beforeTestClass = true) 
  public void startSlaves() {
    startSlave(m_ports[0]);
    startSlave(m_ports[1]);
  }
  ...
  public TestListenerAdapter twoHosts(String strategy) throws IOException {  
    ...
    m_hostFile = createHostProperties(strategy);
    tng.setHostFile(m_hostFile.getAbsolutePath());
    
    XmlSuite suite = createSuite("DistributedV4Suite1", new Class[] { Test1.class, Test2.class });
