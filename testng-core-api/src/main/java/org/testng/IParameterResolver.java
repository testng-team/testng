 ...
 *
 * <ul>
 *   <li><b>Native injection wins.</b> A resolver is never asked about a parameter TestNG supplies
 *       itself -- the first {@link java.lang.reflect.Method} parameter, every {@link ITestContext},
 *       {@link ITestResult} and {@link org.testng.xml.XmlTest} one, and an {@code Object[]} -- so
 *       it cannot displace one. What TestNG does not inject is offered: a second {@code Method}
 *       parameter, or one carrying {@link org.testng.annotations.NoInjection}, before falling
 *       through to the data provider.
 ...
 * -- possibly more than once for the same invocation -- and must be a stable, side effect free
 * decision for a given parameter. {@link #resolveParameter} is called once per parameter per
 * invocation -- so once per data provider row, and once more for every retry attempt, whether or
 * not the row itself is cached -- immediately before the test method runs, which is where a
 * resolver creates whatever per invocation state it owns.
 *
